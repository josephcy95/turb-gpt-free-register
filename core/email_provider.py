...
EMAIL_SOURCE 支持单个或多个来源：
    ...
    "mailnest"
    "cloudmail"
    "outlook,flysms,generic_api,mailnest,cloudmail"          # 按顺序兜底
    ["outlook", "flysms", "generic_api", "mailnest", "cloudmail"]  # 也兼容列表写法
"""
import logging
...
_VALID_SOURCES = ("outlook", "generic_api", "flysms", "cloudflare_domain", "cloudflare", "gptmail", "mailnest", "cloudmail")
...
def _pick_from_source(source: str) -> str:
    ...
    if source == "generic_api":
        ...
        return pick_account().email
    if source == "flysms":
        from core.flysms_mail_client import pick_account
        return pick_account().email
    if source == "mailnest":
...
def resolve_email_source(email: str) -> str:
    ...
    from core import db
    if db.get_flysms_email_by_email(email):
        return "flysms"
...
def wait_for_otp(
    ...
    if source == "generic_api":
        ...
        return fetch_latest_otp(email, after_ts=after_ts, **extra_kwargs)
    if source == "flysms":
        from core.flysms_mail_client import fetch_latest_otp
        return fetch_latest_otp(email, after_ts=after_ts, **extra_kwargs)
    if source == "mailnest":
...
def release_email(email: str, status: str = "available", note: str | None = None) -> str:
    ...
    if source == "gptmail":
        ...
        from core.flysms_mail_client import release_account
        release_account(email, status=status, note=note)
...
def release_email_if_unconsumed(email: str, note: str | None = None) -> bool:
    ...
    if source == "outlook":
        ...
        changed = db.release_unconsumed_flysms_email(email, note=note)
    elif source == "cloudflare_domain":
